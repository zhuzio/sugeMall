...
function shopupgrade(){
    ...
    if(empty($_POST["cate_id"]))err("请确认店铺分类");
    ...
    if($user){
        ...
        $store_name=$model->table('store')->where(array('store_name'=>$store))->select();
        ...
        {
            //查询用户的店铺名
            $user_store=$model->table('store')->where(array('store_id'=>$user['user_id']))->find();
            if($user_store['store_name']!=$store)
            {
                if(!empty($store_name)){
        ...
        $region_name='';
        $condition="id in ({$_POST["province"]},{$_POST['city']},{$_POST['area']})";
        $query=$model->table('sgxt_area')->field('name')->where($condition)->select();
        foreach($query as $k=>$v)
        {
            $region_name.=$v['name'];
        }
        $array['region_id']=$_POST['area'];
        ...
        if($store){
            ...
            if($cate){
                ...
                $model -> table('category_store')-> insert(array('cate_id'=>$_POST['cate_id'],'store_id'=>$store['store_id']));
            }
        }else{
            //获取用户的信息
