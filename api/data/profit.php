...
function depositMonth()
{
    ...
    $model = new M();
    $list = $model->query('select userid as user_id,sum(money) as money,FROM_UNIXTIME(operatortime,\'%Y%m\') times from ecm_sgxt_deposit where userid='.$user['user_id'].' and ispay=1 and type=2 group by times');
    $new = array();
    ...
    outputJson('ok','收益提现月统计',$rs,0,$total);
}
//每月提现明细
function depositMonth_info()
{
    $token = urlencode($_POST['token']);
    $user =checkToken($token);
    if(!$user){
    ...
    $model = new M();
    $totalcount=$model->table('sgxt_deposit')->where('userid='.$user['user_id'].' and ispay=1 and type=2 and operatortime>='.$starttime.' and operatortime<'.$nextmonthtime)->count();
    $list = $model->query('select userid,money,operatortime from ecm_sgxt_deposit where userid='.$user['user_id'].' and ispay=1 and type=2 and operatortime>='.$starttime.' and operatortime<'.$nextmonthtime.' order by operatortime desc limit '.$startcount.','.$pagecount);
    foreach($list as $key=>$val){
        $list[$key]['operatortime']=date('Y-m-d H:i:s',$val['operatortime']);
    }
    $totalpage=ceil($totalcount/$pagecount);
    pageJson('ok',"收益提现月明细",$list,$totalpage);
}
...
function profitMonth()
{
    ...
    outputJson('ok','收益账单月统计',$rs,0,$total);
}
//收益明细：
