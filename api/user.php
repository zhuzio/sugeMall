...

{
    ...
    if($_SESSION['MobileConfirmCode']!=$code)
    ...
    $tjuser=$model->table('member')->where(array('user_name'=>$tjphone,'status'=>conf('site_status')))->find();
...
function jurisdiction(){
	...
	if($user){
		...
		$array=$model
			...
			->query('select user_id,type,province,city,area from ecm_member where user_id = '.$user['user_id'].'');

		if($array['type']=='5'){

			$where +='and ahentarea= '.$array['area'].'';

		}else if($array['type']=='6'){

			$where +='and ahentarea= '.$array['city'].'';

		}else if($array['type']=='7'){

			$where +='and ahentarea= '.$array['province'].'';

		}

		

		$count=$model->query('select count(user_id) as id from ecm_member where type = '.$type.' '.$where.' limit '.$startpage.','.$pagecount.'');
		...
		$data=$model
			...
			->query('select user_id,user_name,real_name,type,reg_time,type,portrait from ecm_member where type = '.$type.'  '.$where.' limit '.$startpage.','.$pagecount.'');
