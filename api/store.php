...
function nearbyshops(){
	if($_POST['type']==""){
		$type='ecm_scategory.is_show=1 and ecm_store.o2o= "offline"';
	}else{
		$type='ecm_scategory.is_show=1 and ecm_store.o2o= "offline" and ecm_scategory.cate_name="'.$_POST['type'].'"';
		$hotsearch=new M();
...
function storeinfo(){
	...
	$model=new M();
		...
		->field('store_id,owner_name,region_name,image_1,image_2,image_3,store_name,address,tel,tel1,tel2,store_logo,description')
...
function classinfo(){
    ...
    $data=$model->query('select cate_id,cate_name from ecm_gcategory where  store_id = 0 and parent_id = 0');
...
function storegoods(){
	...
	if($_POST["cate_name"]==""){
		...
		$where = 'and cate_name like "%'.$_POST["cate_name"].'%"';	
	...
	$startpage=((int)$page-1)*10;	
		...
		foreach ($data as $b=> $val){
			...
			$data[$b]['goods']=$model
					...
					->query('select goods_id,market_price,default_image,goods_name,price from ecm_goods where store_id = '.$val['store_id'].' '.$where.' limit '.$startpage.','.$pagecount.'');
...
function payback(){
    ...
    if($user){
		...
		$orderinfo=$model->table('sgxt_order')->where('orderid=\''.$_POST['order_sn'].'\'')->find();
        ...
        $money=$model
            ...
            ->table('sgxt_order')
			->where(array('orderid'=>$_POST['order_sn']))
