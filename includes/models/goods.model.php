...
class GoodsModel extends BaseModel {
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true) {
        ...
        $goods_pvs_mod = &m('goods_pvs');  // sku  

        $fields .= "g.goods_id,g.point, g.store_id, g.type, g.goods_name, g.cate_id, g.cate_name, g.brand, g.spec_qty, g.spec_name_1, g.spec_name_2, g.if_show, g.closed, g.add_time, g.recommended, g.default_image,g.mall_recommended,g.mall_recommended2,g.isnew,g.isday,g.islike,g.mall_sort_order, " .
        ...
        $tables = "{$this->table} g " .
                ...
                "LEFT JOIN {$goods_pvs_mod->table} gp ON g.goods_id=gp.goods_id " . // sku  
        ...
        foreach ($goods_list as $key => $goods) {
            ...
            * 小阮添加处理图片地址
             */
           $goods_list[$key]['default_image'] = QINIU_URL.$goods['default_image'];
        }
    ...
    function get_info($id) {
        ...
        if ($goods) {
            ...
            * 处理图片显示问题 小阮添加
             */
            $goods['default_image'] =  QINIU_URL.$goods['default_image'];
       
            foreach($goods['_images'] as $key=>$goodsimg){
                $goods['_images'][$key]['image_url'] = QINIU_URL.$goodsimg['image_url'];
                $goods['_images'][$key]['thumbnail'] = QINIU_URL.$goodsimg['thumbnail'];
            }   
