...
EOT;
    ...
    function display($f)
    {
        ...
        $this->assign('qiniu_site_url', QINIU_URL);
    ...
    * 
     */
    function show_success($msg , $url ,$time='3'){
       $this->exe_message($msg , $url ,$time,'success');
    }
    /*
    * 小阮修改提示跳转的方法
    * 
     */
    function show_error($msg , $url ,$time='3'){
         $this->exe_message($msg , $url ,$time,'error');
    }

    function exe_message($msg , $url ,$time, $type){
        if($type=='success'){
            $title = '操作成功';
        }else{
            $title = '操作失败';
        ...
        $this->display('newapp/message.html');  
    ...
    function _get_query_conditions($query_item){
        ...
        foreach ($query_item as $options)
        {
            ...
            if (isset($_GET[$options['name']]) && $_GET[$options['name']] != '')
            {
        ...
        $this->searchQuery =  stripslashes_deep($query);
