...
class PointApp extends MemberbaseApp {
    ...
    function sendPoint(){
        ...
        if(!IS_POST){
            ...
            $order = $this->order_offline->createOrder($get_user_info['user_id'] ,$this->userinfo['user_id'] , 9 , $money , $point , $classid , $remark);
    ...
    public function chechName(){
        echo $this->model->table('member')->where(array('user_name' => trim(I('post.mobile'))))->getField('real_name');
    }
    //验证积分返回商家发送和平台发送
    public function checkPoint(){
        $array['allPoint'] = I('post.point');
    ...
    public function shopSendPointList(){
        $point['sendPoint'] = $this->model->table('sgxt_get_point')->where(array('sendid' => $this->userinfo['user_id']))->sum('point');
        $point['currentPoint'] = $this->model->table('member')->where(array('user_id' => $this->userinfo['user_id']))->getField('pay_point');
        $point['allPoint'] = $point['sendPoint'] +  $point['currentPoint'] ;
        $this->assign('point' , $point);
