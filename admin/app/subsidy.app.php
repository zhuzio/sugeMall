 ...
 *    @usage    none
 */
class SubsidyApp extends BackendApp
{
    /**
     *    管理
     *
    ...
    function index()
    {
        ...
        $totalMoney = 0;
        $rs = $this->model->query('select sum(point) as totalmoney from `ecm_sgxt_subsidy` where 1=1 '.$conditions);

        $orders = $this->model->table('sgxt_subsidy')->find(array(
        ...
        foreach($orders as $key=>$val){
            ...
            $oper = $this->model->table('member')->where('user_id='.$val['opid'])->find1();
            $val['true_name'] = $user['real_name'];
            $val['user_mob'] = $user['user_name'];
            $val['op_name'] = $oper['real_name'];
            $val['add_time'] = date('Y-m-d H:i:s',$val['add_time']);
            $orders[$key] = $val;
        }

        $page['item_count'] = $this->model->table('sgxt_subsidy')->getCount();   //获取统计的数据
        ...
        $this->display('subsidy.index.html');
    }
