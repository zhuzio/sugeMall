...
class ScategoryApp extends BackendApp
{
    ...
    function index()
    {
        ...
        foreach($scategories as $key=>$val){
            $val['is_show_cn'] = $val['is_show'] == 1 ? '显示' : '隐藏';
            $val['o2o_cn'] = $val['o2o'] == 'offline' ? '线下' : '线上';
            $scategories[$key] = $val;
        }
    ...
    function _upload_images($images) {
        ...
        foreach ($images as $image) {
            ...
            $image_urls[$image] = $uploader->save(UPLOAD_DIR, $fileName.$image);
        }
        return $image_urls;
    }
    ...
    function add()
    {
        ...
        else
        {
            ...
            foreach ($images as $image) {
                isset($image_urls[$image]) && $data['thumb'] = $image_urls[$image];
            }
