...
class ArticleApp extends BackendApp
{
    ...
    function add()
    {
        ...
        else
        {
            ...
            $article_logo && $this->_article_mod->edit($article_id, array('article_logo' => $article_logo)); //将logo地址记下
    ...
    function edit()
    {
         ...
         if (!IS_POST)
        {
            ...
            if ($article['article_logo'])
            {
                $article['article_logo']  =   dirname(site_url()) . "/" . $article['article_logo'];
            }
        ...
        else
        {
            ...
            $article_logo   =   $this->_upload_logo($article_id);
            $article_logo && $data['article_logo'] = $article_logo;
            if ($article_logo === false)
            {
                return;
            }
    ...
    function _upload_logo($article_id)
    {
        $file = $_FILES['article_logo'];
        ...
        $uploader->addFile($_FILES['article_logo']);//上传logo
        if (!$uploader->file_info())
        {
            $this->show_warning($uploader->get_error() , 'go_back', 'index.php?app=article&amp;act=edit&amp;id=' . $article_id);
            return false;
        }
        /* 指定保存位置的根目录 */
        ...
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/article', $article_id))  
        {
            return $file_path;
        }
        else
