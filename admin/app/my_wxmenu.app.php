...
class My_wxmenuApp extends BackendApp {
    ...
    function My_wxmenu() {
        ...
        if (empty($ininterface)) {
            $this->show_message(Lang::get('no_ininterface'),  '请先配置', 'index.php?app=my_wxconfig');
            exit;
        }
    }
            ...
            if (count($nextmenu) != 0) {//没有下级栏目
                foreach ($nextmenu as $key2) {
                    if ($key2['weixin_type'] == '0') {
                        ...
                        $kk[] = array('type' => 'view', 'name' => $key2['name'], 'url' => $this->get_authorize_url($key2['likes']));
                    }
                }
                ...
                if ($key['weixin_type'] == '0') {
                    ...
                    $keyword['button'][] = array('type' => 'view', 'name' => $key['name'], 'url' => $this->get_authorize_url($key['likes']));
                }
            }
        }
    ...
    /* 当用户通过按钮访问链接 需要使用回调   获取用户相关信息.以便自动登录 */
    function get_authorize_url($url)
    {
        $user_id = $this->_store_id;
        $wxconfig = $this->my_wxconfig_mod->get_info_user($user_id);
        $url = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=".$wxconfig['appid']."&redirect_uri=".$url."&response_type=code&scope=snsapi_base&state=1#wechat_redirect";
        return $url;
    }
