...
function add_uploadedfile(file_data)
{
    if(file_data.instance == 'goods_image'){
        $('#goods_images').append('<li ectype="handle_pic" file_id="'+ file_data.file_id +'" thumbnail="'+ file_data.thumbnail +'"><input type="hidden" value="'+ file_data.file_id +'" name="goods_file_id[]"/><div class="pic"><img src="<?php echo $this->_var['qiniu_url']; ?>/'+ file_data.thumbnail +'" width="55" height="55" alt="" /><div ectype="handler" class="bg"><p class="operation"><span class="cut_in" ectype="set_cover" ecm_title="设为封面"></span><span class="delete" ectype="drop_image" ecm_title="删除"></span></p></div></div></li>');
                trigger_uploader();
        ...
        $('#desc_images').append('<li file_name="'+ file_data.file_name +'" file_path="'+ file_data.file_path +'" ectype="handle_pic" file_id="'+ file_data.file_id +'"><input type="hidden" name="desc_file_id[]" value="'+ file_data.file_id +'"><div class="pic" style="z-index: 2;"><img src="<?php echo $this->_var['qiniu_url']; ?>/'+ file_data.file_path +'" width="50" height="50" alt="'+ file_data.file_name +'" /></div><div ectype="handler" class="bg" style="z-index: 3;display:none"><img src="<?php echo $this->_var['qiniu_url']; ?>/'+ file_data.file_path +'" width="50" height="50" alt="'+ file_data.file_name +'" /><p class="operation"><a href="javascript:void(0);" class="cut_in" ectype="insert_editor" ecm_title="插入编辑器"></a><span class="delete" ectype="drop_image" ecm_title="删除"></span></p><p class="name">'+ file_data.file_name +'</p></div></li>');
                trigger_uploader();
...
?>
                                        <li ectype="handle_pic" file_id="<?php echo $this->_var['goods_iamge']['file_id']; ?>" thumbnail="<?php echo $this->_var['qiniu_url']; ?>/<?php echo $this->_var['goods_iamge']['thumbnail']; ?>">
                                        <input type="hidden" name="goods_file_id[]" value="<?php echo $this->_var['goods_iamge']['file_id']; ?>">
                                        <div class="pic">
                                            <img src="<?php echo $this->_var['qiniu_url']; ?>/<?php echo $this->_var['goods_iamge']['thumbnail']; ?>" width="55" height="55" />
                            ...
                            <div class="products">
                                <ul>
                                    ...
                                    <li>
                                        <h2>市场价: </h2>
                                        <div class="arrange"><input type="text" name="market_price" value="<?php echo $this->_var['goods']['market_price']; ?>" class="text width_short" />
                                            <span class="gray">为空的话则为输入价格的1.2倍</span></div>
                                    </li>
                                    <li>
                                    ...
                                    <li>
                                        <div class="arrange"><span class="red" style="margin-right:5px;">*</span><span class="gray">会员的价格以原价的折扣来体现,如填写0.85代表原价的8.5折</span></div>
                                    </li>
                                    <li>
                                        <h2  ectype="no_spec_1">赠送积分: </h2>
                                        <div class="arrange"  ectype="no_spec_1"><input name="point"  value="<?php echo $this->_var['goods']['point']; ?>" type="text" class="text width_short" /></div>
                                    </li>
                                    <li>
                                        <div class="arrange"><span class="red" style="margin-right:5px;">*</span><span class="gray">用户购买商品成功后给用户发送相应的消费积分</span></div>
                                    </li>
                                    <li>
                                        <h2>是否开启: </h2>
                                        <div class="arrange">
                                            <span class="distance">
                                                <label><input name="if_open" value="1" <?php if ($this->_var['goods']['if_open']): ?>checked="checked" <?php endif; ?>type="radio"  /> 是</label>
                                                <label><input name="if_open" value="0" <?php if (! $this->_var['goods']['if_open']): ?>checked="checked" <?php endif; ?>type="radio"  /> 否</label>
                                            </span>
                                        </div>
                                    </li>
                                    <?php $_from = $this->_var['goods']['ugrades']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'ugrade');$this->_foreach['fe_ugrade'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['fe_ugrade']['total'] > 0):
    foreach ($_from AS $this->_var['ugrade']):
        $this->_foreach['fe_ugrade']['iteration']++;
?>
                                    <li>
                                        <h2 ><?php echo $this->_var['ugrade']['grade_name']; ?>: </h2>
                                        <input name="grade[]" value="<?php echo $this->_var['ugrade']['grade']; ?>" type="hidden" />
                                        <div class="arrange"><input name="grade_id[]" value="<?php echo $this->_var['ugrade']['grade_id']; ?>" type="hidden" /><input name="grade_discount[]" value="<?php echo ($this->_var['ugrade']['grade_discount'] == '') ? '' : $this->_var['ugrade']['grade_discount']; ?>" type="text" class="text width_short" /></div>
                                    </li>
                                    <?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
...
?>
                                        <li ectype="handle_pic" file_name="<?php echo htmlspecialchars($this->_var['desc_image']['file_name']); ?>" file_path="<?php echo $this->_var['qiniu_url']; ?>/<?php echo $this->_var['desc_image']['file_path']; ?>" file_id="<?php echo $this->_var['desc_image']['file_id']; ?>">
                                        <input type="hidden" name="desc_file_id[]" value="<?php echo $this->_var['desc_image']['file_id']; ?>">
                                            <div class="pic">
                                            <img src="<?php echo $this->_var['qiniu_url']; ?>/<?php echo $this->_var['desc_image']['file_path']; ?>" width="50" height="50" alt="<?php echo htmlspecialchars($this->_var['desc_image']['file_name']); ?>" title="<?php echo htmlspecialchars($this->_var['desc_image']['file_name']); ?>" /></div>
                                            <div ectype="handler" class="bg">
                                            <img src="<?php echo $this->_var['qiniu_url']; ?>/<?php echo $this->_var['desc_image']['file_path']; ?>" width="50" height="50" alt="<?php echo htmlspecialchars($this->_var['desc_image']['file_name']); ?>" title="<?php echo htmlspecialchars($this->_var['desc_image']['file_name']); ?>" />
                                                <p class="operation">
