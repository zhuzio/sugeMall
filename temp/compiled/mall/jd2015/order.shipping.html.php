...
<script type="text/javascript">
                ...
                function set_order_amount_total(){
                    // 通过JS 获取网页信息  重新刷新 金额
                    //运费选择模板ID
                    ...
                    //当前设置积分数 大于用户积分数 则设置所有积分抵扣
                    if(use_integral><?php echo ($this->_var['member_integral'] == '') ? '0' : $this->_var['member_integral']; ?>){
                        $('#use_integral').val('<?php echo ($this->_var['member_integral'] == '') ? '0' : $this->_var['member_integral']; ?>');
                        use_integral = <?php echo ($this->_var['member_integral'] == '') ? '0' : $this->_var['member_integral']; ?>;
                    }
                    //当设置积分数 大于可用积分数 
                    if(use_integral><?php echo ($this->_var['total_integral_max_exchange'] == '') ? '0' : $this->_var['total_integral_max_exchange']; ?>){
                        $('#use_integral').val('<?php echo ($this->_var['total_integral_max_exchange'] == '') ? '0' : $this->_var['total_integral_max_exchange']; ?>');
                        use_integral = <?php echo ($this->_var['total_integral_max_exchange'] == '') ? '0' : $this->_var['total_integral_max_exchange']; ?>;
                    }
                    //积分兑换比例 得出具体抵扣数值
                    var _integral_fee = use_integral*<?php echo ($this->_var['integral_seller'] == '') ? '0' : $this->_var['integral_seller']; ?>;
                    var _amount  = goods_amount + _shipping_fee - _integral_fee;
                    $('#order_amount').html(price_format(_amount));
                }
