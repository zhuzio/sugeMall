...
class ComuploadApp extends StoreadminbaseApp
{
    ...
    {
        file_put_contents('com.txt','123#',FILE_APPEND);
        $this->ComuploadApp();
        $this->qiniu();
    }
    function qiniu(){
    ...
    function remote_image()
    {
        ...
        if (!empty($remote_url))
        {
            if(preg_match("/^(http:\/\/){1,1}.+(gif|png|jpeg|jpg){1,1}$/i", $remote_url))
            {
                ...
                list($qiniu_ret, $qiniu_err) = $this->qiniu_upload->putFile($this->qiniu_token, $new_url, $new_url);
                /* 处理文件入库 */
                if ($err !== null) {
                    echo "<script type='text/javascript'>alert('图片上传到七牛失败');</script>";
                    return false;
                ...
                if ($this->instance == 'goods_image') // 如果是上传商品相册图片
                {
                    ...
                    * 同步到七牛
                     */
                 list($qiniu_ret1, $qiniu_err1) = $this->qiniu_upload->putFile($this->qiniu_token, $new_url, $new_url);
