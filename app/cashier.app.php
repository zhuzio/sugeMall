...
class CashierApp extends ShoppingbaseApp {
    ...
    function index() {
        ...
        if (!$order_info['payment_id']) {
            ...
            /* 否则直接到网关支付 */
            if($order_info['payment_id'] == 17){
                header("Location:/app/reapal/rongpayto.php?dingdan={$order_info['order_sn']}&cz_money=".$order_info['order_amount']);
            }else if($order_info['payment_id'] == 18){
                header("Location:/app/allinpay/allinpay_pc.php?dingdan={$order_info['order_sn']}&cz_money=".$order_info['order_amount']);
            }
    ...
    function goto_pay() {
        ...
        file_put_contents('payment_id.txt',$payment_id);
        if (!$order_id) {
            $this->show_warning('no_such_order');
            return;
        }
