        ...
        switch ($_GET['goods']) {
            case 'groupbuy':
                ...
                $groupbuy_info = $model_groupbuy->get(array(
                    ...
                    'fields' => 'store.store_id, store.store_name, goods.goods_id, goods.point,goods.goods_name, goods.default_image, groupbuy_log.quantity, groupbuy_log.spec_quantity, this.spec_price',
                ));
                ...
                foreach ($spec_quantity as $spec_id => $spec_info) {
                    ...
                    $groupbuy_items[] = array(
                        ...
                        'point' => $groupbuy_info['point'] * $spec_info['qty'],
                    );
                    $amount += $subtotal;
                }
            ...
            default:
                ...
                foreach ($cart_items as $rec_id => $goods) {
                    $goods_point = & m() ->table('goods') ->where(array('goods_id' => $goods['goods_id'])) ->getField('point');
                    $return['quantity'] += $goods['quantity'];                      //商品总量
                    $return['amount'] += $goods['quantity'] * $goods['price'];    //商品总价
                    $return['point'] += $goods_point * $goods['quantity'];
