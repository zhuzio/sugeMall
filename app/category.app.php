...
class CategoryApp extends MallbaseApp
{
    ...
    function index()
    {
        ...
        /* 取得商品分类 */
        if (ECMALL_WAP) {
            $gcategorys = $this->wap_list_gcategory();
        }else{
    ...
    function wap_list_gcategory() {
        $cache_server = & cache_server();
        $key = 'wap_page_goods_category';
        ...
        if ($data === false) {
            ...
            foreach ($gcategories as $gcategory_key => $gcategory) {
                $data[$gcategory_key] = $gcategory;
                $data[$gcategory_key]['children'] = $gcategory_mod->get_list($gcategory['cate_id']);
            }
