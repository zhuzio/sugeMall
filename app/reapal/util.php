...
/**
 ...
 function AESDecryptResponse($encryptKey,$data){
	 $result=decrypt($data,$encryptKey);
	 file_put_contents('reapal_test.txt',$result."\r\n",FILE_APPEND);
	 return $result;
	}
...
function send($paramArr, $url, $apiKey, $reapalPublicKey, $merchant_id){
	if(!empty($paramArr['member_ip']))
	{
		$arr=explode(',',$paramArr['member_ip']);
		$paramArr['member_ip']=$arr[0];
	}
	//生成签名
	$sign = createSign($paramArr,$apiKey);

	$paramArr['sign'] = $sign;
	file_put_contents('reapal_test.txt',json_encode($paramArr)."\r\n",FILE_APPEND);
	//生成AESkey
