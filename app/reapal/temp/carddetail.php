...
<body ng="">
    ...
    <div class="form_warp" style="padding-bottom: 50px">
        ...
        <input type="hidden" name="payform" value="2" />
        ...
        <ul>
            ...
            <li ng-show="elements_flag[4]=='1'" class="">
                <div class="form_item">
                    ...
                    <button class="btn gray veri" style="" type="button" id="sms" >获取</button>
                </div>
            </li>
...
$(function(){
    ...
    $("#pay_submit").on('click',function(){
        ...
        var cvv2 = $("#cvv2").val();
        if(real_name == ''){
            $("#modal-body").html('请填写持卡人真实姓名');
            $("#dialog_mask").show();
            return;
        }
        if(idno == ''){
            $("#modal-body").html('请填写持卡人身份证号码');
            $("#dialog_mask").show();
            return;
        }
        if(mobile == '' || mobile.length != 11){
            $("#modal-body").html('请填写正确的银行预留手机号');
            $("#dialog_mask").show();
            return;
        }
        if(code == '' || code.length != 6){
            $("#modal-body").html('请填写正确的短信验证码');
            $("#dialog_mask").show();
            return;
        }
        if(bank_type == 1){
            if(validate == ''){
                $("#modal-body").html('请填写信用卡有效期');
                $("#dialog_mask").show();
                return;                
            }
            if(cvv2 == ''){
                $("#modal-body").html('请输入卡背面后三位');
                $("#dialog_mask").show();
                return;                
            }
        }
        if(!req_ok){
            $("#modal-body").html('签约未完成');
            $("#dialog_mask").show();
            return;
        }
        $.ajax({
            ...
            data : {
                ...
                'card_no' : bank_num,               //银行卡号
                'owner' : real_name,                //持卡人姓名
                'cert_no' : idno,                   //证件号
                'phone' : mobile,                   //手机号
                'check_code' : code
            },
            ...
            success : function(res){
                $("#mask").hide();
                console.log(res);
                if(res.result_code == '0000'||res.result_code == '3083'){
                    bankName = $("#bank_name").val();
                    bankType = $("#bank_type").val();
                    validate = $("#validate").val();
                    bind_id = $("#bind_id").val();
                    cvv2 = $("#cvv2").val();
                    $.ajax({
                        url : '/api/index.php?n=shop_center&f=bind_bank',
                        type : 'POST',
                        dataType : 'JSON',
                        data : {
                            'order_no':dingdan,                 //订单号
    ...
    function sendverify() {       
        ...
        if(bank_type == 1){
            ...
            $.ajax({
                url : '/app/reapal/credit/creditResult.php',
                type : 'POST',
                dataType : 'JSON',
                data : {
                    'order_no':dingdan,                 //订单号
        ...
        {
            $.ajax({
                url : '/app/reapal/debit/debitResult.php',
                type : 'POST',
                dataType : 'JSON',
                data : {
                    'order_no':dingdan,                 //订单号
